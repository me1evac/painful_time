...
local function clear_task(context)
  if context.state.attack_fireball_task ~= nil then
    context.scheduler:cancel_task(context.state.attack_fireball_task)
    context.state.attack_fireball_task = nil
  end
end
      ...
      for _, player in ipairs(context.boss:get_nearby_players(20)) do
        if valid_player(player) then
          ...
          context.boss:summon_projectile(
            ...
            player_loc,
            0.01,
            { yield = 3.0, incendiary = true, gravity = true, spawn_at_origin = true }
          )
        end
      end
    end)

    context.state.attack_fireball_task = task_id
  end
}
